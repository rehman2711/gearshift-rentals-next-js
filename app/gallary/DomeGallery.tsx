...
function buildItems(pool: ImageItem[] = [], seg: number): ItemDef[] {
  ...
  const usedImages = Array.from({ length: totalSlots }, (_, i) => normalizedImages[i % normalizedImages.length] || { src: "", alt: "" });

  // Try to avoid immediate duplicates where possible
...
export default function DomeGallery({
  ...
  // bind gestures to mainRef (library accepts a ref object as `target`)
  useGesture(
    {
      onDragStart: ({ event }) => {
    ...
    {
      target: mainRef, // library accepts ref object as target
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  ...
                  >
                    <img
                      src={it.src || "/images/cars/ferarri.jpg"}
                      draggable={false}
                      alt={it.alt}
